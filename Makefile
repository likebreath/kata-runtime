...
HYPERVISOR_ICH = cloud-hypervisor
...
HYPERVISORS := $(HYPERVISOR_ACRN) $(HYPERVISOR_FC) $(HYPERVISOR_QEMU) $(HYPERVISOR_QEMU_VIRTIOFS) $(HYPERVISOR_NEMU) $(HYPERVISOR_ICH)
...
ifneq (,$(ICHCMD))
    KNOWN_HYPERVISORS += $(HYPERVISOR_ICH)

    CONFIG_FILE_ICH = configuration-ich.toml
    CONFIG_ICH = $(CLI_DIR)/config/$(CONFIG_FILE_ICH)
    CONFIG_ICH_IN = $(CONFIG_ICH).in

    CONFIG_PATH_ICH = $(abspath $(CONFDIR)/$(CONFIG_FILE_ICH))
    CONFIG_PATHS += $(CONFIG_PATH_ICH)

    SYSCONFIG_ICH = $(abspath $(SYSCONFDIR)/$(CONFIG_FILE_ICH))
    SYSCONFIG_PATHS += $(SYSCONFIG_ICH)
...
ifeq ($(DEFAULT_HYPERVISOR),$(HYPERVISOR_ICH))
    DEFAULT_HYPERVISOR_CONFIG = $(CONFIG_FILE_ICH)
endif
...
$(GENERATED_FILES): %: %.in $(MAKEFILE_LIST) VERSION .git-commit
	$(QUIET_GENERATE)$(SED) \
		...
		-e "s|@CONFIG_ICH_IN@|$(CONFIG_ICH_IN)|g" \
		...
		-e "s|@DEFNETWORKMODEL_ICH@|$(DEFNETWORKMODEL_ICH)|g" \
...
ifneq (,$(findstring $(HYPERVISOR_ICH),$(KNOWN_HYPERVISORS)))
	@printf "\t$(HYPERVISOR_ICH) hypervisor path (ICHPATH) : %s\n" $(abspath $(ICHPATH))
endif
